...
const Features = () => {
  return (
    <section className="py-32 bg-gradient-to-b from-background via-[hsl(220,35%,96%)] to-background relative overflow-hidden" aria-label="Platform features">
      ...
      <div className="container relative mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-20 animate-fade-in-up">
          <div className="inline-block px-4 py-2 bg-secondary/10 border border-secondary/30 rounded-full text-secondary font-medium mb-6">
            Why Wealthconomy?
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {features.map((feature, index) => (
            <Card 
              key={index} 
              className="border-border bg-card/50 backdrop-blur-sm hover:border-secondary/40 transition-all duration-500 hover:shadow-[0_8px_30px_hsl(45,98%,58%/0.12)] hover:-translate-y-3 group animate-scale-in cursor-pointer relative overflow-hidden"
            ...
            >
              ...
              <CardContent className="pt-8 pb-8 space-y-4 relative z-10">
                <div className={`h-14 w-14 rounded-2xl bg-gradient-to-br ${feature.gradient} flex items-center justify-center transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 group-hover:shadow-lg`}>
                  <feature.icon className="h-7 w-7 text-foreground transition-all duration-500 group-hover:scale-110" />
                </div>
                <h3 className="text-2xl font-bold group-hover:text-secondary transition-colors duration-300">{feature.title}</h3>
