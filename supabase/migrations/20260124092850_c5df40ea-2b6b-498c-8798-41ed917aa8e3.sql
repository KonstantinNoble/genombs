...
BEGIN
  ...
  -- Model usage statistics (FIXED: gptMini instead of gpt5, added sonarReasoning)
  SELECT json_build_object(
    'gptMini', COUNT(*) FILTER (WHERE 'gptMini' = ANY(selected_models)),
    ...
    'sonarReasoning', COUNT(*) FILTER (WHERE 'sonarReasoning' = ANY(selected_models))
  )
  INTO v_model_usage
  FROM validation_analyses
  WHERE user_id = p_user_id;
