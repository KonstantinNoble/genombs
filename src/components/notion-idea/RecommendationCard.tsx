      ...
      <CardContent className="p-4 sm:p-6">
        ...
        <div className="flex items-start gap-3 mb-4">
          <div className="flex-1 min-w-0">
            ...
            <h4 className="text-base sm:text-lg font-semibold mb-2 break-words leading-tight">{name}</h4>
        ...
        <div className="mb-4 p-4 rounded-lg bg-muted/50">
          <div className="prose prose-sm max-w-none">
            <ReactMarkdown
              ...
              components={{
                h2: ({ children }) => (
                  <h2 className="text-base sm:text-xl font-semibold mt-4 mb-3 text-foreground">{children}</h2>
                ),
                h3: ({ children }) => (
                  <h3 className="text-sm sm:text-lg font-semibold mt-3 mb-2 text-foreground">{children}</h3>
                ),
                h4: ({ children }) => (
                ...
                ul: ({ children }) => (
                  <ul className="list-disc pl-5 mb-3 space-y-1.5 text-foreground text-sm">{children}</ul>
                ),
                ol: ({ children }) => (
                  <ol className="list-decimal pl-5 mb-3 space-y-1.5 text-foreground text-sm">{children}</ol>
                ),
                li: ({ children }) => (
                ...
                pre: ({ children }) => <pre className="overflow-x-auto rounded-md p-3 bg-muted text-sm">{children}</pre>,
                code: ({ children }) => <code className="break-all">{children}</code>,
                table: ({ children }) => <div className="overflow-x-auto"><table className="min-w-full text-left text-sm">{children}</table></div>,
        ...
        {recommendation.detailedSteps && recommendation.detailedSteps.length > 0 && (
          <Accordion type="single" collapsible className="mb-3">
            <AccordionItem value="steps">
              <AccordionTrigger className="text-sm py-3">Implementation Steps</AccordionTrigger>
              <AccordionContent>
        ...
        {recommendation.prerequisites && recommendation.prerequisites.length > 0 && (
          <Accordion type="single" collapsible className="mb-3">
            <AccordionItem value="prerequisites">
              <AccordionTrigger className="text-sm py-3">Prerequisites</AccordionTrigger>
              <AccordionContent>
        ...
        {recommendation.metrics && recommendation.metrics.length > 0 && (
          <Accordion type="single" collapsible className="mb-3">
            <AccordionItem value="metrics">
              <AccordionTrigger className="text-sm py-3">Success Metrics</AccordionTrigger>
              <AccordionContent>
                <ul className="list-disc list-inside space-y-1.5 text-sm text-muted-foreground">
        ...
        {/* Source Info */}
        <div className="flex flex-wrap gap-2 text-xs text-muted-foreground mt-3 pt-3 border-t border-border/50">
