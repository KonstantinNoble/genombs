...
const WebsiteProfileCard = ({ profile, compact }: WebsiteProfileCardProps) => {
    ...
    return (
      <Card className="border-border bg-card">
        <CardContent className="p-3 flex items-center gap-3">
          <ScoreRing score={overall_score} size={36} />
  ...
  return (
    <Card className={`border-border bg-card ${is_own_website ? "ring-1 ring-primary/30" : ""}`}>
      <CardContent className="p-5 space-y-4">
        <div className="flex items-start gap-4">
          <ScoreRing score={overall_score} />
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2">
              <h3 className="text-base font-bold text-foreground">{profile_data.name}</h3>
              <Badge variant={is_own_website ? "default" : "outline"} className="text-[10px] shrink-0">
                {is_own_website ? "Your Site" : "Competitor"}
              </Badge>
            </div>
